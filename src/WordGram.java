...
public class WordGram {
	...
	public String wordAt(int index) {
		if (index < 0 || index >= length())throw new IndexOutOfBoundsException("bad index in wordAt "+index);
	...
	*/
	...
	@Override
	public int hashCode() {
		if(myHash == 0)myHash = this.toString().hashCode();
	...
	public WordGram shiftAdd(String last) {
		...
		WordGram added = new WordGram(myWords, 0, myWords.length);

		for(int i = 0; i<myWords.length-1; i++){
			added.myWords[i] = myWords[i+1];
		added.myWords[myWords.length-1] = last;
		}
		return added;
	}
	...
	public String toString() {
		if(myToString == null) myToString = String.join(" ", myWords);
